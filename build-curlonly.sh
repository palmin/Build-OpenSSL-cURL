...
MACOS_X86_64_VERSION="13.1"			# Empty = use host version
MACOS_ARM64_VERSION="13.1"			# Min supported is MacOS 11.0 Big Sur
...
#SKIP(curl-only) ./openssl-build.sh -v "$OPENSSL" $engine $colorflag $catalyst $sslv3 $OSARGS
cd ..

## LibSSH2 Build
cd libssh2
echo -e "${bold}Building LibSSH2${normal}"
#SKIP(curl-only) ./libssh2-build.sh
cd ..
